$(function () {
    ...
    $('.geo_reg_input').on('change', function () {
        var elem = $(this);
        if (elem.is(":checked")) {
            console.log(elem.sublings('.region-wrap'));
            elem.children('.geo_input').each(function (el) {
                el.attr('checked', 'checked')
            });
        }
        else
            elem.children('.geo_input').removeAttr('checked');
    });
    ...
    function to_geoClick() {
        ...
        var lem = $(this).siblings('.lemma').data('lem');
        if (!confirm('Переместить слово <b>' + lem + '</b> в гео данные')) {
            return;
        }
        var $this = $(this);
        $.ajax({
